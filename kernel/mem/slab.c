...
int slab_alloc_chunk(struct slab *slab)
{
	...
	page = page_alloc_with_retry(ALLOC_ZERO);
	assert_oom(page);

	void* page_vaddr = page2kva(page);

	info = (struct slab_info*) (((uintptr_t) page_vaddr) + slab->info_off);
	info->slab = slab;
	list_init(&info->free_list);

	for (size_t i = 0; (i < slab->obj_size * slab->count); i += slab->obj_size) {
		struct slab_obj *obj = page_vaddr + i;
		list_init(&obj->node);
...
void slab_free_chunk(struct slab *slab, struct slab_info *info)
{
	...
	struct page_info *page = page_lookup(kernel_pml4, (void*) ((uintptr_t) info - slab->info_off), NULL);
